...
The pinned vendor remote TUI is not assumed protocol-only merely because its argv contains `--remote`. The original guardian may attach it only inside an OS-enforced Linux mount namespace created before vendor exec. That namespace exposes the exact verified runtime and canonical Codex credential directory read-only at their original absolute paths, leaves only the explicitly required non-credential client state writable, drops mount capabilities before vendor code runs, and sets `no_new_privs`. The guardian verifies each live credential path non-destructively through nofollow identity, read access, symlink-target, and mount read-only evidence. Destructive write, truncate, rename-over, unlink, and symlink-replacement probes operate only on fabricated sentinel files placed beside the credential views before the read-only remount; they never target either live credential path or bytes. Only after every sentinel mutation is refused does the guardian bind and supervise the exact client PID and plausible descendants. A copied credential, discretionary file mode, argv classification, or post-launch observation is not sufficient enforcement. Missing user/mount-namespace support, failed bind/remount, retained mount capability, failed refusal probe, unsupported platform, or uncertain teardown returns exit 3 before the remote client starts. The namespace is an availability constraint, not a fallback boundary: there is no unprotected remote attachment.
...
Regression tests cover mutable-link topology, state migration/quiescence, CLI/ACP/web/daemon launch paths, isolated bundle cleanup, `ihar check --diff`, MCP golden renders, and settings tamper checks. Remote attachment tests exercise the real namespace helper boundary with fabricated credentials: non-destructive identity/read/mount checks cover the exact runtime link and canonical absolute target, while direct write, truncate, rename-over, unlink, and symlink replacement target only fabricated adjacent sentinels and must all fail before the fake remote client can mark startup. A supported host must still permit the required daemon socket and explicitly declared client-state writes. Unsupported or unavailable namespace enforcement must produce the bounded exit-3 refusal. The final stable source state receives the full suite once. Live validation uses metadata-only status and human-run account authorization; it never prints a credential payload. The observed pre-existing Codex materialized file cannot be used for a live success claim until its separate recovery checkpoint is approved and completed. No test pass alone substitutes for observing both vendor launches and the intended Codex re-authentication behavior.

...
The API tuple is environmental only when the probe also proves that dispatch was absent. An observed hook marker, command sentinel or sandbox status outranks the later API error and leaves the case failed, because some measured execution occurred and its failed assertion cannot be erased. The same rule applies to Claude's live-hook and native-sandbox turns. Any other terminal reason, or an absent hook marker without the exact API tuple or bounded legacy environment reason, remains a real failure; hook discovery, trust or non-dispatch is never converted to `unmeasured` merely because the vendor exits nonzero.

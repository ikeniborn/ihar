...
The Codex auth-owner lease is separate from the project state lock. It admits one ihar-managed Codex writer globally across CLI, ACP, daemon/web and isolated guest, with a two-second admission bound. A client may attach only to the exact verified managed daemon owner; an independently selected generation or guest cannot write concurrently. The daemon retains ownership after its initiating CLI exits. Unknown process identity, opaque external consumers or unproved child quiescence leave the lease unavailable; neither a lock timeout nor a stale record permits an unleased launch. Claude has no Codex credential lease.

The correction specified in `docs/superpowers/specs/2026-09-22-codex-auth-claude-runtime-recovery-design.md` is pending implementation: one continuous guardian must acquire this lease before any ihar-managed Codex executable, including prelaunch hook-trust checks, and keep it through daemon or guest teardown and credential reconciliation. Check, dry-run, conformance/install, and switch may run without it only when their path cannot start Codex. A crash leaves a blocking owner record until quiescence is proven. The current implementation has not yet demonstrated this boundary and must not be treated as meeting it.
...
For Codex, the isolated guest receives a private credential copy only under the global auth-owner lease. After guest shutdown, ihar checks authenticated bundle ownership, nofollow topology, the unchanged canonical baseline and guest quiescence before a changed credential can be published through a recoverable transaction. Previous and candidate bytes remain recoverable until durability is proved. Missing or ambiguous guest output does not log out the host; failed checks retain evidence and block reuse. This does not authorize adopting an older materialized runtime credential.

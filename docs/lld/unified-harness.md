...
| `auth/claude/.credentials.json`, `auth/codex/auth.json` | globally shared mutable vendor logins; Codex's file is the sole durable owner | vendors through `manifests/mutable-links.json`; ihar stages Codex authentication before guarded publication |
| `auth/codex/.owner.json`, `.owner.lock`, `.guardian.sock`, `staging/`, `recovery/` | private schema-2 guardian state, authenticated control endpoint, login stages and retained credential candidates; metadata never contains credential bytes | continuous Codex guardian, mode-`0700` ancestry and mode-`0600` leaves |
...
Codex authentication does not run against the runtime link. `ihar codex -- login …` routes only authentication verbs to a private mode-`0700` staging `CODEX_HOME`; an ordinary session keeps the verified runtime link to the canonical store file. A first login may publish a uniquely owned staged file after quiescence and provenance checks. Re-authentication of an existing canonical credential requires direct human approval; `--assume-yes` cannot grant it, and its previous bytes remain recoverable until durable publication is verified. Vendor account action may invalidate an old token remotely even when those bytes remain. An already materialized runtime `auth.json` is preserved, never adopted or deleted automatically, and needs a separate user-approved recovery proposal.
...
**`<config-hash>` is the first 8 hex characters of `sha256` over everything that decides how the vendor behaves**: profile name, effective masking level, gateway mode, sandbox mode, MCP strictness, the rendered hook manifest digest, the registry digest, the pinned vendor version, the effective MCP identity (§7.2), the stable semantic digest of the validated persistent-state manifest, and the canonical identity of every `runtime:true` asset entry plus its nofollow topology in the actual store. Asset bytes do not select a runtime because the home links to the store, but declared source/target/kind/required semantics and installed presence do; adding or removing an optional installed source therefore selects a new generation. Two launches with the same effective configuration share a runtime home and race on no rendered file; two launches with different configurations get different directories. Codex nonetheless has one global credential writer across those homes (§4.3). A runtime home is written once, under the state lock, and is thereafter **immutable**: a configuration change produces a new directory rather than a rewrite.

The Codex guardian is deliberately acquired before this runtime path exists. Its schema-2 record starts in `pending`, then the guarded child binds the exact absolute runtime and configuration hash over the inherited authenticated socket. A copied descriptor number, environment marker or owner ID cannot perform that bind. The same record follows preflight, hook seal and verification, foreground work, daemon or guest lifetime, and final credential reconciliation; ownership is never handed from one phase to another.
...
| `ihar_effective_mcp_identity` | hashes the validated, selected non-secret MCP render before runtime selection; launch and `ihar check --diff` use the same input |
| `ihar.codex.guardian.run` / authenticated `request` operations | creates the schema-2 owner before the guarded child starts, authenticates the inherited Unix descriptor and process ancestry, binds runtime/children/daemon/guest, and retains ownership through proven quiescence and credential reconciliation |
...
The Codex auth-owner lease is separate from the project state lock. One Linux child-subreaper guardian acquires it before the first ihar-managed Codex executable and retains it across CLI, ACP, daemon/web, runtime seal and trust verification, ordinary check, conformance/install/update, Codex switch distillation and isolated guest return. Admission precedes store/install locks. A check may remain metadata-only only when routing proved Codex absent; a binary that appears later is not probed. A copied environment marker or reusable owner ID has no authority: guarded operations require the inherited authenticated socket and verified process ancestry. There is no release/reacquire phase and no ownership handoff.

Admission is global, bounded to two seconds, and fail-closed. An exact remote client attaches only through the original daemon guardian and only for the same runtime/generation; a new independent launch cannot share the writer. The guardian survives its initiating shell while a verified daemon, attached client or guest remains. Crash, PID reuse, opaque external consumers, unsafe control state, unacknowledged guest publication or unproved quiescence leaves a durable `blocked` owner record. Unsupported descendant supervision returns exit 3 before vendor execution; neither a lock timeout nor a stale record permits an unguarded fallback. Claude has no Codex credential lease.

### 4.4 Rendered configuration

Claude managed keys include `hooks`, `enabledPlugins`, `statusLine`, `extraKnownMarketplaces`, `sandbox` and `_iharGateway`. Because a runtime home is immutable, there is no merge-on-launch step: `ConfigRenderer` produces the complete `settings.json` from the store template plus the managed block, and user-owned keys come from `manifests/config/claude/settings.json` and from `.ihar_config`. Reuse and `ihar check --diff` share a comparator: Claude's observed top-level strings `theme` and `tui` are vendor-owned and ignored, while changed managed fields and unknown additional fields are drift. `tui` was measured, not assumed: Claude 2.1.274 wrote `"tui": "fullscreen"` into a live runtime `settings.json`, and the next launch refused the home as drifted. Diagnostics report the first field path, never its value. An edit to a managed field under `standard` does not silently change the configuration hash or select a new home; it makes reuse fail closed.
...
The auth-owner record binds the daemon's process start identity, socket identity, runtime and generation. Daemon startup is a child of the already running guardian; the daemon never acquires a second lease. Attached clients ask the owner-only control socket to pass their standard-I/O descriptors to that same guardian, which starts and tracks them. Its guardian continues to hold the lease after the initiating CLI exits; daemon stop releases it only after the daemon, attached clients and plausible descendants are quiescent. A stale or unverifiable owner record is not permission to start another daemon.
...
`mcp-matcher-fires` is deliberately end-to-end despite its historical name: the marker alone proves matcher selection, while the server sentinel proves that execution continued through `tools/call`. Codex's MCP approval default is `auto`; an unannotated tool requires approval, and the conformance turn's global `approval_policy="never"` then rejects it after `PreToolUse`. The disposable one-tool server therefore sets `default_tools_approval_mode="approve"`. That setting is confined to the staged probe configuration and keeps the case non-interactive without changing any rendered production server policy.
...
**A failed case says why without storing dynamic text.** Every case record keeps the fixed detail `<name>: <status>`. A failed case also carries one token from the closed reason vocabulary: `vendor-quota-exhausted`, `vendor-unauthenticated`, `vendor-unreachable`, `vendor-api-error`, `vendor-rejected-argv`, `vendor-exited-nonzero`, `hook-never-fired`, `sentinel-missing`, `decision-not-recorded`, `rewrite-not-applied`, `timeout`, `case-raised`, or `unclassified`. Human output prints only the status, case name and optional reason token. Case sentences, vendor output and model output are never persisted or printed, as §14 requires.

**A case that could not be measured is not a case that failed.** A quota, a missing login, an unreachable endpoint and a vendor API error before tool dispatch say nothing about whether a vendor honours a hook decision. Recording them as failures blocked an install that had nothing wrong with it — observed when this project's own debugging exhausted a Codex quota — so those turns are recorded `unmeasured` with one word from the reason vocabulary. Claude 2.1.274 reports this boundary in its JSON result as `type: result`, `subtype: success`, `is_error: true` and `terminal_reason: api_error`. The runner recognizes only that exact structural tuple and never persists or prints the result text. Parsed JSON and JSON-lines stdout is never phrase-scanned, so model, account and result payload text cannot choose a reason; legacy phrase matching is limited to non-JSON stdout and bounded stderr.

The API tuple is environmental only when the probe also proves that dispatch was absent. An observed `PreToolUse` marker, command sentinel or sandbox status outranks the later API error and leaves the case failed, because some measured execution occurred and its failed assertion cannot be erased. `session-start-context` is the deliberate exception: its `SessionStart` marker occurs before the model/API turn and proves hook loading only, so the target sentinel is that case's sole evidence of downstream delivery and tool dispatch. The same rule applies to Claude's remaining live-hook and native-sandbox turns. Any other terminal reason, or an absent dispatch marker without the exact API tuple or bounded legacy environment reason, remains a real failure; hook discovery, trust or non-dispatch is never converted to `unmeasured` merely because the vendor exits nonzero.

The consequence is asymmetric on purpose. An install activates through this path only when at least one required case is `unmeasured` with an allowed environmental reason, every other required case passed, and no case — including an extra diagnostic case — failed. It discards the record as proof and adds the vendor to a staged warning list that is consumed after activation and never published, with a warning naming what happened. An enforced profile still refuses to launch that vendor because the proof record is absent: an unmeasured case proves nothing. `ihar check --conformance` is what turns it into evidence once the environment allows. One real failure anywhere in the record makes the record a failure again, whatever else went unmeasured.
...
Before a runtime path is selected, the validated registry, profile, required-variable presence and resolved non-secret render values produce one deterministic effective MCP identity per vendor. Secret forwarded values are excluded. Launch and `ihar check --diff` call the same identity function and include its result in the generation hash; a selected server or endpoint change selects a new immutable generation. Rendering verifies final bytes against that identity rather than rewriting a published generation for the current environment.

`ihar check --diff` prints the selected generation and fixed reason categories rather than configuration or environment values. Codex configuration mismatches are classified by the already-rendered MCP tables versus other managed settings; Claude settings use the shared projection comparator, where only the top-level strings `theme` and `tui` are vendor-owned and every managed or unknown field remains drift. Codex auth diagnostics reuse the runtime-link and auth-owner checks: mutable-link status is bounded to `missing`, `unverified`, `materialized`, `unsafe type` or `valid`, while owner status is bounded to `unverified`, `no recorded owner`, `busy`, `blocked` or `quiescence unverified`. A durable schema-2 `blocked` status is trusted only after the canonical bounded owner-record validator accepts the complete record; malformed or truncated metadata remains `unverified` even when it names schema 2 and `blocked`. Credential bytes, owner IDs, paths from private metadata and environment values are never rendered.
...
For Codex, the isolated guest receives a private credential copy only under the original continuous guardian. Only its `auth.json` is placed in the writable state image; policy, configuration, hooks and store assets remain read-only. Bundle registration precedes Firecracker, binds the exact owner-only state-image inode, seed digest and canonical baseline, and keeps the verified image descriptor open. There is no guest owner-ID handoff or second acquisition.

After `sync` and verified Firecracker shutdown, the guardian extracts only the guest credential through that retained descriptor, checks bundle ownership, nofollow topology, unchanged canonical baseline and guest quiescence, then performs the recoverable publication transaction. A changed sole-owner refresh may publish automatically; re-authentication and pre-existing materialized credentials still require their separate human checkpoints. Publication remains `published-pending` until the requester acknowledges the one-time success token and the guardian rechecks exact candidate and canonical identities. Lost acknowledgment, missing/ambiguous output, baseline drift, extraction failure or rollback uncertainty retains candidate and prior bytes and leaves the owner blocked; it never becomes logout or silent cleanup.
...
`ihar check --diff` reports each selected runtime generation. The selection includes effective MCP identity and uses the same hash input as launch; the diff does not calculate a second identity. Once the existing file comparison finds a difference, diagnostic classification examines the desired and active Codex `config.toml` MCP tables to distinguish `mcp-render-drift` from `managed-setting-drift`; a missing file is `rendered-config-missing`, while a malformed or purely textual mismatch is `rendered-config-drift`, never a guessed MCP cause. Claude's separate MCP file is identified directly. Claude settings use the same field-path comparator as reuse, so a string `theme` or `tui` is ignored while managed or unknown changes name only a field path. For an existing selected Codex runtime, the diagnostic reports bounded mutable-link and auth-owner/lease categories, including a distinct schema-2 `blocked` state, without displaying credential bytes, environment values, or owner metadata. A materialized `auth.json` is reported as preserved, not adopted or deleted, and requiring user-approved recovery; the check never repairs it.
...
| auth owner | `tests/test_codex_auth_owner.py`, `tests/test_codex_auth_lease.py` | nofollow staging/publication and rollback; schema-2 owner before child start; descriptor/process-bound admission; prelaunch refusal; daemon/remote client lifetime; auxiliary check/conformance/install/switch admission; crash, PID reuse and unsupported-platform exit 3 |
...
| diagnostics | `tests/test_contracts.sh`, `tests/test_mcp.sh`, `tests/test_state.sh`, `tests/test_lifecycle.sh`, `tests/test_claude_compare.py` | bounded generation/MCP/managed-setting/auth-owner categories; no secret values; materialized credential preservation; MCP-derived generation selection; top-level Claude `theme` and `tui` exception with managed and unknown drift still closed |
...
| Codex auth guardian | busy/blocked/unverifiable owner; unsupported supervision; failed daemon attachment or guest reconciliation | fail-closed, preserve owner and recovery evidence | 3 |
